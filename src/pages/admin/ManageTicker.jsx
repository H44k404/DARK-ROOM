...
const ManageTicker = () => {
    const [config, setConfig] = useState({ enabled: false, message: '' });
                    ...
                    <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-8 space-y-10 relative overflow-hidden">
                        ...
                        <div className="relative">
                            ...
                            <label className="flex items-center gap-6 cursor-pointer group">
                                <div className={`w-20 h-10 rounded-full transition-all duration-500 relative ${config.enabled ? 'bg-red-600 shadow-lg shadow-red-200' : 'bg-gray-100'}`}>
                                    <input
                                        type="checkbox"
                                ...
                                <span className={`text-xl font-bold tracking-tight transition-colors duration-300 ${config.enabled ? 'text-red-600' : 'text-gray-400 group-hover:text-primary-black'}`}>
                                    {config.enabled ? 'LIVE BROADCAST ACTIVE' : 'TRANSMISSION STANDBY'}
                                </span>
                            </label>
                        </div>
                        ...
                        <div className="relative">
                            ...
                            <textarea
                                ...
                                rows="5"
                                className="w-full bg-gray-50/50 border border-gray-100 rounded-3xl p-6 text-xl md:text-2xl font-bold tracking-tight text-primary-black placeholder-gray-200 focus:ring-2 ring-red-50 focus:border-red-100 transition-all resize-none"
                                placeholder="BREAKING: ENTER CRITICAL UPDATES..."
                            />
                            <div className="flex items-center gap-2 mt-4 text-xs font-bold text-gray-400 uppercase tracking-tighter">
                                <HiInformationCircle className="text-lg text-blue-500" />
                        ...
                        <div className="border border-gray-50 rounded-2xl p-4 bg-gray-50/30 min-h-[120px] flex items-center justify-center overflow-hidden relative">
                            {config.enabled ? (
                                <div className="w-full transform scale-105">
                                    <BreakingNewsTicker message={config.message} enabled={config.enabled} />
                                </div>
                            ) : (
